...
(() => {
  ...
  const PLAYER_HIT_RADIUS = 0.70;     // unchanged collision feel
  const PLAYER_VISUAL_SCALE = 0.60;   // v14 visual size
  const OBS_VISUAL_SCALE = 0.60;      // v14 visual size
  ...
  const AVOID_SCAN_RADIUS = 15.0;        // look ahead for nearby observers
  const AVOID_CRITICAL_RADIUS = 2.8;     // emergency reaction zone
  const AVOID_PREDICT_SEC = 1.10;        // predict observer positions ahead
  ...
  function chooseAvoidance(p,s,now){
    ...
    for(const o of observers){
      ...
      // Crowd awareness: estimate which side of the racing line is less occupied.
      const side=dx*s.nx+dy*s.ny;
      const danger=Math.max(0,AVOID_SCAN_RADIUS-dist) +
                   Math.max(0,AVOID_SCAN_RADIUS-futureDist)*1.25;
      if(side>=0) rightDanger+=danger;
      else leftDanger+=danger;

      const score=dist*0.58+futureDist*1.18;
    ...
    if(!threat || threatScore>13.0){
      p.avoidThreatId=-1;
      return null;
    }

    // Exactly one 90% decision when the nearest threat changes.
    // No frame-by-frame rerolls, so this remains a genuine ~90% reaction rate.
    if(p.avoidThreatId!==threat.id){
      p.avoidThreatId=threat.id;
      p.avoidWillDodge=Math.random()<0.90;
    }
    if(!p.avoidWillDodge) return null;

    // Move toward the side with less predicted observer traffic.
    const openSide = leftDanger<=rightDanger ? -1 : 1;
    const r=Math.random();

    if(immediate){
      if(r<0.18) return {mode:"stop",side:openSide,strength:1.0};
      if(r<0.68) return {mode:"diagonal",side:openSide,strength:1.0};
      return {mode:"zigzag",side:openSide,strength:1.0};
    }

    if(r<0.12) return {mode:"stop",side:openSide,strength:0.90};
    if(r<0.64) return {mode:"diagonal",side:openSide,strength:0.96};
    if(r<0.88) return {mode:"zigzag",side:openSide,strength:0.93};
    return {mode:"wide",side:openSide,strength:0.88};
  }
  ...
  function drawObserver(o,view){
    ...
    const r=Math.max(2.52,view.scale*0.72*OBS_VISUAL_SCALE);
    ctx.save();
    ctx.translate(x,y);
    ctx.fillStyle="#d6e8ff";
    ctx.strokeStyle="#5f89ad";
    ctx.lineWidth=Math.max(0.84,view.scale*.11*OBS_VISUAL_SCALE);
  ...
  function drawPlayer(p,view,rank){
    ...
    const r=Math.max(6.0,view.scale*1.15*PLAYER_VISUAL_SCALE);
