...
window.__OBSERVER_FM_BUILD__ = BUILD_ID;
  ...
  function setupLeagueHeat100(){
    ...
    if(!mapResult100.ok){
      console.error("[v1.80.0] league map activation failed",mapResult100);
    ...
    if(activeMapId770!==nextMapId100 || currentMap770().id!==nextMapId100){
      console.error("[v1.80.0] league map id mismatch",{requested:nextMapId100,active:activeMapId770,current:currentMap770().id});
      league100.phase="bracket";
      return false;
    }
...
applyMapSet776();
  ...
  function maybeSpecialControl197(p,now,info,maxLane){
    ...
    // v1.80.0: flashy controls should be rare on an empty road, but a little more
    ...
    const chance=nearby===0
      ? (.0010+creativity*.0020)
      : Math.min(.032,.010+creativity*.014+Math.min(4,nearby)*.0020);
