...
function updateDestinyPlayer183(p,now,dt){
  ...
  function buildExecutionPlan1101(p,now,info,corridor){
    ...
    const stageTimeouts=[
      threat.count>=5?170:145,
      threat.count>=5?190:165,
  ...
  function executionSafetyChanged1101(p,info,plan,now){
    ...
    if(suddenNear)return "FULL";
    if(sideFlip||newCrowd)return "PARTIAL";
    return "NONE";
  }
  ...
  function executeEscapePlan1101(p,now,info){
    ...
    // Actual lane-arrival completion.
    const reached=dist<.20;

    // Per-stage timeout prevents getting stuck forever.
    const timeout=Number(plan.stageTimeouts?.[phase])||170;
    const timedOut=now-(plan.phaseStartedAt||plan.createdAt)>=timeout;
...
function seasonCardHtml(p){
  ...
  function applyPatch1103(){
    window.__OBSERVER_FM_V1103__={
      executionCompletionFix:true,
      phaseCompletionByLaneArrival:true,
      perStageTimeout:true,
      partialReplan:true,
      fullReplanOnlyOnSuddenNear:true,
      emergencyMultiCandidate:true,
      emergencyCandidateCount:6,
      executionCompletionTelemetry:true,
      codeAudit1103:true
    };
  }
  applyPatch1103();
