...
(() => {
  ...
  const PLAYER_VISUAL_SCALE = 0.75;   // v14 visual size
  ...
  const AVOID_SCAN_RADIUS = 19.0;        // look ahead for nearby observers
  const AVOID_CRITICAL_RADIUS = 4.8;     // emergency reaction zone
  const AVOID_PREDICT_SEC = 1.55;        // predict observer positions ahead
  const AVOID_REACTION_CHANCE = 0.985;  // much stronger reaction rate
  const AVOID_SAFE_BUFFER = 2.25;        // extra body-size safety margin
  ...
  function chooseAvoidance(p,s,now){
    ...
    if(!p.avoidWillDodge && !immediate) return null;
  ...
  function updatePlayer(p, now, dt){
    ...
    if(avoid){
      const evadeHalf=Math.max(2.6,widths[si]*0.94);
      if(avoid.mode==="stop"){
        speedMul=0;
      }else if(avoid.mode==="diagonal"){
        targetOff += avoid.side*evadeHalf*1.08*avoid.strength;
        speedMul=1.01;
      }else if(avoid.mode==="zigzag"){
        targetOff += avoid.side*evadeHalf*0.90 +
          Math.sin(now*0.031+p.index)*evadeHalf*0.38;
        speedMul=0.96;
      }else if(avoid.mode==="wide"){
        targetOff += avoid.side*evadeHalf*1.04;
        speedMul=0.94;
      }
    }
  ...
  function drawPlayer(p,view,rank){
    ...
    const r=Math.max(7.5,view.scale*1.15*PLAYER_VISUAL_SCALE);
