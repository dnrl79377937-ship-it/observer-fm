...
(() => {
  ...
  function observerCountForMap791(m=currentMap770()){
    ...
    if(id==="double_hairpin")return 130;  // Three-Leaf Clover
    if(id==="cliff_hanger")return 80;    // 스카이클리프
    if(id==="skyway")return 110;          // 스페이스
    if(id==="s_map")return 80;           // 네온드리프트
    if(id==="star_fish")return 150;
    if(id==="triple_diamond")return 160;  // 데스티니게이트
    if(id==="ice_ring")return 180;
    if(id==="neon_city")return 180;       // 하트
    if(id==="desert_oasis")return 180;
    return 100;
  }
...
function seasonCardHtml(p){
  ...
  function v36SelfAudit(){
    ...
    if(observerCountForMap791(MAP_DEFINITIONS_770.double_hairpin)!==130)issues.push("Three-Leaf Clover observer count 130");
    if(observerCountForMap791(MAP_DEFINITIONS_770.cliff_hanger)!==80)issues.push("스카이클리프옵저버80");
    if(observerCountForMap791(MAP_DEFINITIONS_770.skyway)!==110)issues.push("스페이스옵저버110");
    if(observerCountForMap791(MAP_DEFINITIONS_770.s_map)!==80)issues.push("네온드리프트옵저버80");
    if(observerCountForMap791(MAP_DEFINITIONS_770.star_fish)!==150)issues.push("스타피쉬옵저버150");
    if(observerCountForMap791(MAP_DEFINITIONS_770.triple_diamond)!==160)issues.push("데스티니게이트옵저버160");
    if(observerCountForMap791(MAP_DEFINITIONS_770.ice_ring)!==180)issues.push("아이스크라운옵저버180");
    if(observerCountForMap791(MAP_DEFINITIONS_770.neon_city)!==180)issues.push("하트옵저버180");
    if(observerCountForMap791(MAP_DEFINITIONS_770.desert_oasis)!==180)issues.push("사막오아시스옵저버180");
  ...
  (function(){
    ...
    for(const id of expected){
      ...
      img.onerror=()=>{report.assetChecks[id]={ok:false,image:m.image};report.ok=false;console.error('[v1.82.0] map image load failed',id,m.image);};
      img.src=m.image;
    }
  })();
