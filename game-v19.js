...
(() => {
  ...
  const PLAYER_VISUAL_SCALE = 1.03125;   // v14 visual size
  ...
  const AVOID_PREDICT_SEC = 2.65;        // predict observer positions ahead
  ...
  const INSIDE_CORNER_STRENGTH = 0.93; // Kart-style inside apex bias
  ...
  function makePlayers(){
    return names.map((name,i)=>{
      ...
      return {
        ...
        speed: (9.50 + paceNorm*0.42 + Math.random()*0.08) * 1.15,
  ...
  function updatePlayer(p, now, dt){
    ...
    let targetOff=optimalOffsetFor(p);
      ...
      if(insideSide!==0 && turnPower>0.055){
        const halfRoad=Math.max(1.5,widths[si]*0.50);
        const apexOff=insideSide*halfRoad*INSIDE_CORNER_STRENGTH;
        const apexBlend=Math.min(0.97,0.58+turnPower*1.72);
  ...
  function drawPlayer(p,view,rank){
    ...
    const r=Math.max(12.925,view.scale*1.48*PLAYER_VISUAL_SCALE);
