...
(() => {
  ...
  const PLAYER_HIT_RADIUS = 0.56;     // unchanged collision feel
  const PLAYER_VISUAL_SCALE = 0.9590625;   // v14 visual size
  ...
  const AVOID_SCAN_RADIUS = 31.0;        // look ahead for nearby observers
  const AVOID_CRITICAL_RADIUS = 8.0;     // emergency reaction zone
  const AVOID_PREDICT_SEC = 2.75;        // predict observer positions ahead
  const AVOID_REACTION_CHANCE = 0.999;  // much stronger reaction rate
  const AVOID_SAFE_BUFFER = 5.0;
  const AVOID_LANE_LOOKAHEAD = 2.60;   // compare future lane safety
  const INSIDE_CORNER_STRENGTH = 0.985; // Kart-style inside apex bias
  const PLAYER_LANE_MICRO_SPREAD = 0.13; // tiny stable spread; avoids overlap jitter
  ...
  function updatePlayer(p, now, dt){
    ...
    let targetOff=optimalOffsetFor(p);
      ...
      if(insideSide!==0 && turnPower>0.055){
        const halfRoad=Math.max(1.6,widths[si]*0.54);
        const apexOff=insideSide*halfRoad*INSIDE_CORNER_STRENGTH;
        const apexBlend=Math.min(0.995,0.68+turnPower*1.92);
      ...
      if(Math.abs(futureInside)>0.10){
        const halfRoad2=Math.max(1.6,widths[si]*0.55);
        const futureApex=futureInside*halfRoad2*0.96;
        targetOff=targetOff*0.26+futureApex*0.74;
      }
    ...
    if(!avoid && p.controlMode==="zigzag"){
      ...
      speedMul=0.875+controlSkill*0.065;
          ...
          // Stable micro-spread between racers: avoids exact center stacking without
      // changing race order or creating player-vs-player collision physics.
      targetOff += ((p.index%4)-1.5)*PLAYER_LANE_MICRO_SPREAD;
targetOff=Math.max(-half,Math.min(half,targetOff));
  ...
  function updateCamera(dt){
    ...
    const a=Math.min(0.18,dt*0.0042);
  ...
  function loop(ts){
    ...
    const dt=Math.min(24,Math.max(0,ts-lastTs));
    lastTs=ts;
  ...
  function drawPlayer(p,view,rank){
    ...
    const r=Math.max(12.02025,view.scale*1.48*PLAYER_VISUAL_SCALE);
