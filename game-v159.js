...
window.__OBSERVER_FM_BUILD__ = BUILD_ID;
  ...
  const MAP_POOL_770=[
    ...
    {id:"ice_ring",slot:3,name:"Ice Crown",en:"Ice Crown",theme:"Frozen Crown Canyon",
    ...
    {id:"desert_oasis",slot:4,name:"Desert Oasis",en:"Desert Oasis",theme:"Desert & Oasis",
    ...
    {id:"star_fish",slot:8,name:"Star Fish",en:"Star Fish",theme:"Tropical Star Island",
    ...
    {id:"triple_diamond",slot:9,name:"Destiny Gate",en:"Destiny Gate",theme:"Heaven vs Hell Destiny Gate",
  ...
  function mapStart770(){ const m=currentMap770(); if(m.id==='double_hairpin')return {x:73.5,y:162.6}; return {x:m.start?.x??31.05,y:m.start?.y??132.55}; }
  ...
  function neonDriftMapId108(){
    // v1.57.0: ObserverS is the S-map only. Heart (neon_city) must never be selected as ObserverS.
    return MAP_DEFINITIONS_770?.s_map ? "s_map" : (MAP_POOL_770?.[0]?.id || null);
  }
...
function calibratedFastCorridor79(si){
  ...
  function forceStartCenter625(p){
    ...
    // v1.58.0: Clover must spawn on the artwork's LEFT yellow pad.
    // Do not inherit any retired slot-6/shared-gate/central spawn coordinate.
...
function applyMapSet776(){
  ...
  const meta={
    s_map:{slot:1,name:"ObserverS",en:"ObserverS",theme:"Blue Neon S Course",tags:["기본","S자","네온"],image:"map_v672_equal_medium_start_goal.png?v=776-neon-drift"},
    star_fish:{slot:2,name:"Star Fish",en:"Star Fish",theme:"Tropical Star Island",tags:["기본","별모양","한바퀴"],image:"map_star_fish_791.png?v=803-theme-tile"},
    ice_ring:{slot:3,name:"Ice Crown",en:"Ice Crown",theme:"Frozen Crown Canyon",tags:["기본","M자","아이스"],image:"map_ice_m_787.png?v=793-ice-crown-clean",special:{shortcuts:false,obstacles:false,wideRoad:false,multiRoute:false,verticality:false}},
    desert_oasis:{slot:4,name:"Desert Oasis",en:"Desert Oasis",theme:"Desert Ruins Oasis",tags:["기본","사막","한바퀴"],image:"map_desert_oasis_899.png?v=803-theme-tile",special:{shortcuts:false,obstacles:false,wideRoad:true,multiRoute:false,verticality:false}},
    neon_city:{slot:5,name:"Heart",en:"Heart",theme:"Cherry Blossom Heart",tags:["기본","하트","한바퀴"],image:"map_heart_7891_clean.png?v=803-theme-tile",special:{shortcuts:false,obstacles:false,wideRoad:true,multiRoute:false,verticality:false}},
    double_hairpin:{slot:6,name:"세잎 클로버",en:"Three-Leaf Clover",theme:"Tropical Clover Circuit",tags:["세잎클로버","우측루프","반시계"],image:"map_clover_157.png?v=1510",special:{shortcuts:false,obstacles:false,wideRoad:false,multiRoute:false,verticality:false}},
    skyway:{slot:7,name:"Space",en:"Space",theme:"Deep Space Narrow Run",tags:["좁은길","직선","우주"],image:"map_space_894.png?v=803-theme-tile",special:{shortcuts:false,obstacles:false,wideRoad:false,multiRoute:false,verticality:true}},
    ...
    triple_diamond:{slot:9,name:"Destiny Gate",en:"Destiny Gate",theme:"Heaven vs Hell Destiny Gate",tags:["데스티니게이트","2스타트","천국vs지옥"],image:"map_destiny_gate_8113.png?v=817-destiny-gate",special:{shortcuts:false,obstacles:false,wideRoad:true,multiRoute:true,verticality:false}}
  };
...
applyMapSet776();
  ...
  function applyGeometry777(){
    ...
    set777("double_hairpin",{
      image:"map_clover_157.png?v=1510",imageSize:{w:1122,h:1402},
      logicalSize:{w:142.451,h:178},
  ...
  function applyPatch787(){
    ...
    const black=MAP_DEFINITIONS_770.double_hairpin;
    if(black){
      black.image="map_clover_157.png?v=1510";
...
function updateDestinyPlayer183(p,now,dt){
  ...
  // v1.59.0: Clover owns an isolated ordered path.  Do not use the global
  // racing spline here: legacy map patches can rebuild that global spline and
  // make a freshly spawned Clover racer snap to an old central route.
  function cloverPathInfo159(p){
    const m=currentMap770();
    const pts=Array.isArray(m.route770)?m.route770:[];
    if(pts.length<2)return null;
    const lens=[]; let total=0;
    for(let i=0;i<pts.length-1;i++){
      const dx=pts[i+1][0]-pts[i][0],dy=pts[i+1][1]-pts[i][1];
      const len=Math.hypot(dx,dy)||1e-6; lens.push({a:pts[i],b:pts[i+1],len,start:total}); total+=len;
    }
    const pointAt=(pr)=>{
      pr=Math.max(0,Math.min(total,Number(pr)||0));
      let seg=lens[lens.length-1];
      for(const q of lens){if(pr<=q.start+q.len){seg=q;break;}}
      const t=Math.max(0,Math.min(1,(pr-seg.start)/seg.len));
      const x=seg.a[0]+(seg.b[0]-seg.a[0])*t,y=seg.a[1]+(seg.b[1]-seg.a[1])*t;
      const ux=(seg.b[0]-seg.a[0])/seg.len,uy=(seg.b[1]-seg.a[1])/seg.len;
      return {x,y,ux,uy};
    };
    let prog=Number.isFinite(p._v120Prog)?p._v120Prog:0;
    prog=Math.max(0,Math.min(total,prog));
    return {destiny:false,clover159:true,total,prog,pointAt};
  }

  function pathInfo120(p){
    if(currentMap770().id==="double_hairpin"){
      const ci=cloverPathInfo159(p);
      if(ci)return ci;
    }
    if(currentMap770().id==="triple_diamond"){
      destinyPath813(p);
...
function seasonCardHtml(p){
  ...
  function applyPatch817(){
    ...
    if(m){
      ...
      m.en='Destiny Gate';
      m.image='map_destiny_gate_8113.png?v=817-destiny-gate';
      m.imageFallback791='map_destiny_gate_8113.png?v=810-triple-diamond';
...
function observerCountForMap120(map){
  ...
  function applyPatch130(){
    ...
    Object.assign(clover,{
      ...
      tags:["세잎클로버","우측루프","반시계"],
      image:"map_clover_157.png?v=1510",imageSize:{w:1536,h:1536},logicalSize:{w:178,h:178},
      route770:[
  ...
  function applyPatch131(){
    const clover=MAP_DEFINITIONS_770.double_hairpin;
    if(!clover)return;
    clover.image="map_clover_157.png?v=1510";
  ...
  window.__OBSERVER_FM_V132__={
    ...
    cloverAsset:"map_clover_157.png?v=1510",compatibilityMapId:"double_hairpin"
  };
...
(function applyPatch1460(){
  ...
  // Traced against the visible centre of the light-green road in map_clover_157.png?v=1510.
...
// ============================================================
// v1.51.0 — CLOVER ASSET AUTHORITY + JITTER CLEANUP
// A fresh asset filename is used everywhere so bracket/minimap/gameplay cannot
// fall back to a cached one-stem Clover. Runtime geometry is rebuilt from the
// two-stem point-to-point course. Clover racers stay on the centre spline.
// ============================================================
(function applyPatch1510(){
  const clover=MAP_DEFINITIONS_770.double_hairpin;
  if(!clover)return;
  clover.name='Three-Leaf Clover'; clover.en='Three-Leaf Clover';
  clover.image='map_clover_157.png?v=1510';
  ...
  clover.route770=route; clover.widths770=new Array(route.length-1).fill(10.8);
  clover.start={x:74,y:166}; clover.goal={x:104,y:166};
  clover.safeZones={start:{x0:68,y0:159,x1:80,y1:174},goal:{x0:98,y0:159,x1:110,y1:174}};
  clover.sharedGate778=false; clover.courseType775='point-to-point';
  clover.finishRule775='end-gate'; clover.lapRequired775=false;
  clover.strictRoadFollow778=true; clover.strictNoChord795=true;
  clover.roadFollowMode778='clover-two-stem-centre-v151';
  clover.extraRoads771=[]; clover.forbiddenZones770=[]; clover.roadMask770=undefined;
  const centre=densifyLine772(route,.14);
  clover.racingSpline770=centre; clover.globalOptimal770=centre;
  clover.lockOptimalExecution784=true; clover.optimizedSplineAuthority783=true;
  clover.racingLineMode772='clover-left-start-full-road-right-goal-v1.51.0';
  if(currentMap770()?.id==='double_hairpin'){
    route=clover.route770.map(p=>[...p]); widths=[...clover.widths770];
    GLOBAL_OPTIMAL_LINE_710=clover.globalOptimal770; RACING_SPLINE_720=clover.racingSpline770;
    rebuildRouteGeometry770(); rebuildGlobalOptimal770(); rebuildRacingSpline770();
...
(function applyPatch1520(){
  ...
  if(!clover)return;
  clover.name='Three-Leaf Clover'; clover.en='Three-Leaf Clover';
  clover.image='map_clover_157.png?v=1520';
  clover.imageSize={w:1536,h:1536}; clover.logicalSize={w:178,h:178};
  clover.miniCrop={x:0,y:0,w:178,h:178};
...
// ============================================================
// v1.57.0 — FINAL NEW-MAP AUTHORITY / CLOVER CLEANUP
// Audited from the repository ZIP.  All historical Clover image references now
// resolve to map_clover_157.png, and this final definition is the only runtime
// geometry used after startup.
// Route: yellow-left START -> 9 -> 11 -> 12 -> 1 -> 3 -> 5 -> green-right GOAL.
// ============================================================
(function applyPatch1570(){
  const clover=MAP_DEFINITIONS_770.double_hairpin;
  if(!clover)return;
  const r=[
    [74,162],[74,153],[74,143],[74,132],[73,121],[67,113],
    [57,109],[46,108],[35,105],[25,99],[18,91],[14,81],[14,71],[18,62],[25,55],[35,50],[46,48],[57,51],[66,57],
    [71,51],[68,42],[67,33],[69,24],[74,17],[81,12],[89,10],
    [97,12],[104,17],[109,24],[111,33],[110,42],[106,51],
    [112,57],[121,51],[132,48],[143,50],[153,55],[160,62],[164,71],[164,81],[160,91],[153,99],[143,105],[132,108],[121,109],[111,113],
    [105,121],[104,132],[104,143],[104,153],[104,162]
  ];
  Object.assign(clover,{
    name:'Three-Leaf Clover',en:'Three-Leaf Clover',
    image:'map_clover_157.png?v=1570',imageSize:{w:1254,h:1254},logicalSize:{w:178,h:178},miniCrop:{x:0,y:0,w:178,h:178},
    route770:r,widths770:new Array(r.length-1).fill(10.2),
    start:{x:73.5,y:162.6},goal:{x:104.4,y:162.7},
    safeZones:{start:{x0:68.2,y0:156.2,x1:79.8,y1:168.2},goal:{x0:98.2,y0:156.2,x1:109.8,y1:168.2}},
    ...
    strictRoadFollow778:true,strictNoChord795:true,roadFollowMode778:'route-center-hard',
  ...
  clover.racingSpline770=centre;clover.globalOptimal770=centre;
  clover.lockOptimalExecution784=true;clover.optimizedSplineAuthority783=true;
  clover.racingLineMode772='yellow-9-11-12-1-3-5-green-v157';
  POINT_TO_POINT_MAPS_775.add('double_hairpin');CIRCUIT_MAPS_775.delete('double_hairpin');
  // Correct stale QA metadata that used to demand the retired Clover/Black-hole flags.
  clover.geometryReady=true;clover.approvedImageShape772=true;
  if(currentMap770()?.id==='double_hairpin'){
    activeMap770=clover;MAP_W=178;MAP_H=178;resizeObserverGrid777();
    route=r.map(p=>[...p]);widths=[...clover.widths770];
    GLOBAL_OPTIMAL_LINE_710=centre;RACING_SPLINE_720=centre;
    rebuildRouteGeometry770();rebuildGlobalOptimal770();rebuildRacingSpline770();
    applyMapImage770(clover);
    for(const p of players||[]){forceStartCenter625(p);p._v120Prog=0;p._splineProg720=0;p._splineFloor754=0;p._lane120=0;p._lineOffset720=0;}
  }
  window.__OPF_BUILD__='v1.58.0';
})();


// ============================================================
// v1.59.0 — CLOVER FIRST-FRAME SNAP FIX
// Spawn is already correct on the yellow pad. The bug was the first movement
// frame consuming a stale global spline. Clover now advances only on route770.
// ============================================================
(function applyPatch1590(){
  const c=MAP_DEFINITIONS_770.double_hairpin;if(!c)return;
  c.start={x:73.5,y:162.6};c.goal={x:104.4,y:162.7};
  c.sharedGate778=false;c.hideRuntimeSharedGate7992=true;c.observerCount797=150;
  c.roadFollowMode778='isolated-clover-route-v159';
  // Keep current racers at the legitimate yellow spawn; reset progress only.
  if(currentMap770()?.id==='double_hairpin'){
    for(const p of players||[]){
      p._v120Prog=0;p._v120Total=NaN;p._splineProg720=0;p._splineFloor754=0;
      p._lane120=0;p._lineOffset720=0;p._laneVelSec181=0;p._laneVel120=0;
    }
  }
  window.__OPF_BUILD__='v1.59.0';
})();
