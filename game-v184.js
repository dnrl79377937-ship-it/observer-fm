...
(() => {
  ...
  function observerCountForMap791(m=currentMap770()){
    ...
    if(id==="double_hairpin")return 90;   // Three-Leaf Clover
    if(id==="cliff_hanger")return 90;    // 스카이클리프
    if(id==="skyway")return 90;           // 스페이스
    if(id==="s_map")return 80;           // 네온드리프트
    if(id==="star_fish")return 110;
    if(id==="triple_diamond")return 130;  // 데스티니게이트
    if(id==="ice_ring")return 160;
    if(id==="neon_city")return 140;       // 하트
    if(id==="desert_oasis")return 140;
    return 100;
  }
...
window.__OBSERVER_FM_BUILD__ = BUILD_ID;
  ...
  function resetLeague100(){
    ...
    // v1.71.0: every current map can be banned; no protected/fixed test maps.
    // With 9 maps and 2 distinct bans, the remaining 7 maps become sets 1-7
    // in a fresh random order for each bracket reset.
    const allMapIds171=MAP_POOL_770.map(m=>m.id).filter(Boolean);
    const banPool171=shuffle100(allMapIds171);
    league100.bannedMaps.A=banPool171[0]??null;
    league100.bannedMaps.B=banPool171.find(id=>id!==league100.bannedMaps.A)??null;
    league100.setMaps=shuffle100(allMapIds171.filter(id=>
      id!==league100.bannedMaps.A && id!==league100.bannedMaps.B
    )).slice(0,7);
  ...
  function setupLeagueHeat100(){
    ...
    if(!mapResult100.ok){
      console.error("[v1.81.0] league map activation failed",mapResult100);
    ...
    if(activeMapId770!==nextMapId100 || currentMap770().id!==nextMapId100){
      console.error("[v1.81.0] league map id mismatch",{requested:nextMapId100,active:activeMapId770,current:currentMap770().id});
      league100.phase="bracket";
      return false;
    }
...
applyMapSet776();
  ...
  function controlVariant112(p,now){
    ...
    if(now>=(p._variantUntil112||0)){
      ...
      // v1.81.0: calm empty-road driving. Random flourishes are rare unless observers are nearby.
      const localCount181=localObservers723(p,9.5).length;
      const variantChance181=localCount181===0 ? .008 : Math.min(.075,.025+localCount181*.012);
      const roll=Math.random();
      if(roll<variantChance181){
  ...
  function maybeSpecialControl197(p,now,info,maxLane){
    ...
    // v1.81.0: flashy controls should be rare on an empty road, but a little more
    // likely when observers are nearby (while still blocking them at emergency distance).
    ...
    const nearby=Math.max(0,Number(sensor.count||0));
    const chance=nearby===0
      ? (.00025+creativity*.00065)
      : Math.min(.036,.011+creativity*.016+Math.min(4,nearby)*.0022);
  ...
  function renderMiniMap(force=false){
    ...
    // v1.81.0: Space keeps the same game map/camera; only its minimap is displayed wider.
    const miniWrap181=mc.closest?.(".minimap-wrap");
    const spaceMini181=currentMap770().id==="skyway";
    mc.style.width=spaceMini181?"176px":"126px";
    mc.style.height="123px";
    if(miniWrap181) miniWrap181.style.width=spaceMini181?"184px":"134px";
    ...
    const hideRuntimeGates171=!!currentMap770().hideRuntimeStartFinish171;
...
function seasonCardHtml(p){
  ...
  function applyPatch108(){
    ...
    window.__OBSERVER_FM_V108__={
      mapBanEnabled:true,
      neonBanProtected:false,
      set1NeonFixed:false,
      allMapsBannable171:true,
      allSetMapsRandom171:true,
      neonMapId:neon
    };
  }
  applyPatch108();
  ...
  function v36SelfAudit(){
    ...
    if(observerCountForMap791(MAP_DEFINITIONS_770.double_hairpin)!==90)issues.push("Three-Leaf Clover observer count 90");
    if(observerCountForMap791(MAP_DEFINITIONS_770.cliff_hanger)!==90)issues.push("스카이클리프옵저버90");
    if(observerCountForMap791(MAP_DEFINITIONS_770.skyway)!==90)issues.push("스페이스옵저버90");
    if(observerCountForMap791(MAP_DEFINITIONS_770.s_map)!==80)issues.push("네온드리프트옵저버80");
    if(observerCountForMap791(MAP_DEFINITIONS_770.star_fish)!==110)issues.push("스타피쉬옵저버110");
    if(observerCountForMap791(MAP_DEFINITIONS_770.triple_diamond)!==130)issues.push("데스티니게이트옵저버130");
    if(observerCountForMap791(MAP_DEFINITIONS_770.ice_ring)!==160)issues.push("아이스크라운옵저버160");
    if(observerCountForMap791(MAP_DEFINITIONS_770.neon_city)!==140)issues.push("하트옵저버140");
    if(observerCountForMap791(MAP_DEFINITIONS_770.desert_oasis)!==140)issues.push("사막오아시스옵저버140");
  ...
  (function(){
    ...
    if(issues.length)console.error("[v1.72.0] map registry audit",issues);
  ...
  (function(){
    ...
    Object.assign(clover,{
      ...
      image:'map_clover_164.png?v=1720',imageSize:{w:1254,h:1254},logicalSize:{w:178,h:178},miniCrop:{x:0,y:0,w:178,h:178},
      ...
      sharedGate778:false,hideRuntimeSharedGate7992:true,hideRuntimeStartFinish171:true,courseType775:'point-to-point',finishRule775:'end-gate',lapRequired775:false,
      strictRoadFollow778:true,strictNoChord795:true,roadFollowMode778:'clover-v162-single-centre-route',
      extraRoads771:[],forbiddenZones770:[],roadMask770:undefined,observerCount797:90,
    ...
    window.__OBSERVER_FM_MAP_CLEANUP_1630__={
      ...
      cloverStart:{...clover.start},cloverGoal:{...clover.goal},observerCount:90
    };
  })();
  ...
  (function(){
    ...
    window.__OBSERVER_FM_MAP_CLEANUP_1660__={version:'v1.74.0',issues,obsoleteClover130131Removed:true,obsoleteClover142145Removed:true,lateMapBehaviorPreserved:true,retiredSlot6FlagsPurged:true};
    if(issues.length) console.error('[v1.74.0] late-map audit',issues);
  })();

  // v1.72.0: randomized 9-map ban/set board; no fixed Clover test set.
  resetLeague100();
  // v1.72.0 regression guard: all nine maps participate in bans; after two distinct bans,
  // the seven remaining maps must populate sets 1-7 with no fixed Clover slot.
  (function(){
    const ids=MAP_POOL_770.map(m=>m.id).filter(Boolean);
    const unique=new Set(ids);
    const bans=[league100?.bannedMaps?.A,league100?.bannedMaps?.B].filter(Boolean);
    const sets=(league100?.setMaps||[]).filter(Boolean);
    const issues=[];
    if(ids.length!==9||unique.size!==9) issues.push('map-pool-not-9-unique');
    if(bans.length!==2||bans[0]===bans[1]) issues.push('bans-not-2-distinct');
    if(sets.length!==7||new Set(sets).size!==7) issues.push('sets-not-7-unique');
    if(sets.some(id=>bans.includes(id))) issues.push('banned-map-in-sets');
    window.__OBSERVER_FM_V173_AUDIT__={version:'v1.74.0',ids,bans,sets,issues};
    if(issues.length) console.error('[v1.74.0] league map audit',issues);
  })();

  // ============================================================
  // v1.74.0 — 9-MAP RUNTIME / LOGIC AUDIT
  // Read-only validation: does not select/reset a map or alter race state.
  // ============================================================
  (function(){
    const expected=['s_map','star_fish','ice_ring','desert_oasis','neon_city','double_hairpin','skyway','cliff_hanger','triple_diamond'];
    const report={version:'v1.74.0',ok:true,maps:{},globalIssues:[],assetChecks:{}};
    const finitePoint=p=>Array.isArray(p)&&p.length>=2&&Number.isFinite(+p[0])&&Number.isFinite(+p[1]);
    const dist=(a,b)=>Math.hypot((+a[0])-(+b[0]),(+a[1])-(+b[1]));
    const ids=MAP_POOL_770.map(m=>m&&m.id).filter(Boolean);
    if(ids.length!==9||new Set(ids).size!==9) report.globalIssues.push('map-pool-must-be-9-unique');
    for(const id of expected){
      const m=MAP_DEFINITIONS_770[id], issues=[];
      if(!m){ report.maps[id]={ok:false,issues:['missing-definition']}; continue; }
      const rr=Array.isArray(m.route770)?m.route770:[];
      const sp=Array.isArray(m.racingSpline770)?m.racingSpline770:[];
      if(!m.geometryReady) issues.push('geometry-not-ready');
      if(!String(m.image||'').match(/\.png(?:\?|$)/i)) issues.push('missing-png-image');
      if(rr.length<2||rr.some(p=>!finitePoint(p))) issues.push('invalid-route');
      if(sp.length<2||sp.some(p=>!finitePoint(p))) issues.push('invalid-racing-spline');
      if(Array.isArray(m.widths770)&&m.widths770.length && m.widths770.some(w=>!Number.isFinite(+w)||+w<=0)) issues.push('invalid-width');
      const logical=m.logicalSize||{w:172,h:178};
      if(!Number.isFinite(+logical.w)||!Number.isFinite(+logical.h)||+logical.w<=0||+logical.h<=0) issues.push('invalid-logical-size');
      const oc=observerCountForMap791(m);
      if(!Number.isFinite(+oc)||+oc<0) issues.push('invalid-observer-count');
      if(id!=='triple_diamond'){
        if(!m.start||!Number.isFinite(+m.start.x)||!Number.isFinite(+m.start.y)) issues.push('invalid-start');
        if(!m.goal||!Number.isFinite(+m.goal.x)||!Number.isFinite(+m.goal.y)) issues.push('invalid-goal');
        if(rr.length>1&&m.start&&dist(rr[0],[m.start.x,m.start.y])>35) issues.push('route-start-far-from-start');
        if(rr.length>1&&m.goal&&dist(rr[rr.length-1],[m.goal.x,m.goal.y])>35) issues.push('route-end-far-from-goal');
      } else {
        if(!Array.isArray(m.dualStarts810)||m.dualStarts810.length!==2) issues.push('destiny-dual-starts');
        if(!Array.isArray(m.destinyRoads813)||m.destinyRoads813.length<2) issues.push('destiny-roads');
      }
      if(id==='double_hairpin'){
        if(m.sharedGate778!==false||m.lapRequired775!==false||m.courseType775!=='point-to-point') issues.push('clover-mode');
        if(+oc!==150) issues.push('clover-observers-not-150');
        if(!m.hideRuntimeStartFinish171) issues.push('clover-runtime-gates-visible');
      }
      report.maps[id]={ok:issues.length===0,name:m.name,image:m.image,routePoints:rr.length,splinePoints:sp.length,observerCount:oc,issues};
    }
    for(const id of ids) if(!expected.includes(id)) report.globalIssues.push('unexpected-map:'+id);
    report.ok=report.globalIssues.length===0&&Object.values(report.maps).every(x=>x.ok);
    window.__OBSERVER_FM_V174_AUDIT__=report;
    if(!report.ok) console.error('[v1.74.0] 9-map logic audit',report);

    // Non-blocking browser asset load check for all nine map images.
    for(const id of expected){
      const m=MAP_DEFINITIONS_770[id]; if(!m||!m.image) continue;
      const img=new Image();
      img.onload=()=>{report.assetChecks[id]={ok:true,w:img.naturalWidth,h:img.naturalHeight};};
      img.onerror=()=>{report.assetChecks[id]={ok:false,image:m.image};report.ok=false;console.error('[v1.83.0] map image load failed',id,m.image);};
      img.src=m.image;
    }
  })();
