...
(() => {
  ...
  const AVOID_SCAN_RADIUS = 22.0;        // look ahead for nearby observers
  const AVOID_CRITICAL_RADIUS = 5.8;     // emergency reaction zone
  const AVOID_PREDICT_SEC = 1.85;        // predict observer positions ahead
  const AVOID_REACTION_CHANCE = 0.995;  // much stronger reaction rate
  const AVOID_SAFE_BUFFER = 3.20;        // extra body-size safety margin
  ...
  function updatePlayer(p, now, dt){
    ...
    if(avoid){
      const evadeHalf=Math.max(3.2,widths[si]*0.98);
      if(avoid.mode==="stop"){
        speedMul=0;
      }else if(avoid.mode==="diagonal"){
        targetOff += avoid.side*evadeHalf*1.18*avoid.strength;
        speedMul=1.01;
      }else if(avoid.mode==="zigzag"){
        targetOff += avoid.side*evadeHalf*1.00 +
          Math.sin(now*0.031+p.index)*evadeHalf*0.38;
        speedMul=0.96;
      }else if(avoid.mode==="wide"){
        targetOff += avoid.side*evadeHalf*1.14;
        speedMul=0.94;
      }
    }
  ...
  function drawPlayer(p,view,rank){
    ...
    const r=Math.max(9.4,view.scale*1.48*PLAYER_VISUAL_SCALE);
