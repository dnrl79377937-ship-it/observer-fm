...
(() => {
  ...
  const PLAYER_HIT_RADIUS = 0.82;     // unchanged collision feel
  const PLAYER_VISUAL_SCALE = 0.60;   // requested: 50% smaller icon
  const OBS_VISUAL_SCALE = 0.60;      // requested: 50% smaller observer
  const OBS_SPEED_RATIO = 0.90;         // observer speed ≈ 70% of player speed
  ...
  const AVOID_SCAN_RADIUS = 14.0;        // look ahead for nearby observers
  const AVOID_CRITICAL_RADIUS = 3.5;     // emergency reaction zone
  const AVOID_PREDICT_SEC = 1.15;        // predict observer positions ahead
  ...
  function spawnObservers(){
    ...
    for(let i=0;i<OBSERVER_COUNT;i++){
      ...
      const speed=baseSpeed*(0.97+Math.random()*0.06);
  ...
  function chooseAvoidance(p,s,now){
    ...
    // One avoidance decision per short threat window.
    // This prevents frame-by-frame rerolls from turning 90% into effectively 100%.
    if(now>=p.avoidDecisionUntil){
      p.avoidWillDodge = Math.random() < 0.90;
      p.avoidDecisionUntil = now + 720;
    }
    ...
    for(const o of observers){
      ...
      if(now>=o.phaseUntil){
        if(o.phase==="move"){
          ...
          const angle=Math.random()*Math.PI*2;
          o.speed=(9.72*OBS_SPEED_RATIO)*(0.97+Math.random()*0.06);
