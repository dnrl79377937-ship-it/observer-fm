...
(() => {
  ...
  const PLAYER_VISUAL_SCALE = 0.9375;   // v14 visual size
  ...
  const AVOID_SCAN_RADIUS = 27.0;        // look ahead for nearby observers
  const AVOID_CRITICAL_RADIUS = 7.0;     // emergency reaction zone
  const AVOID_PREDICT_SEC = 2.25;        // predict observer positions ahead
  const AVOID_REACTION_CHANCE = 0.998;  // much stronger reaction rate
  const AVOID_SAFE_BUFFER = 4.20;
  const AVOID_LANE_LOOKAHEAD = 2.60;   // compare future lane safety
  const INSIDE_CORNER_STRENGTH = 0.84; // Kart-style inside apex bias
  ...
  function updatePlayer(p, now, dt){
    ...
    let targetOff=optimalOffsetFor(p);
      ...
      if(insideSide!==0 && turnPower>0.055){
        const halfRoad=Math.max(1.4,widths[si]*0.46);
        const apexOff=insideSide*halfRoad*INSIDE_CORNER_STRENGTH;
        const apexBlend=Math.min(0.92,0.48+turnPower*1.55);
    ...
    if(avoid){
      const evadeHalf=Math.max(3.8,widths[si]*1.00);
      if(avoid.mode==="stop"){
        speedMul=0;
      }else if(avoid.mode==="diagonal"){
        targetOff += avoid.side*evadeHalf*1.28*avoid.strength;
        speedMul=1.01;
      }else if(avoid.mode==="zigzag"){
        targetOff += avoid.side*evadeHalf*1.12 +
          Math.sin(now*0.031+p.index)*evadeHalf*0.38;
        speedMul=0.96;
      }else if(avoid.mode==="wide"){
        targetOff += avoid.side*evadeHalf*1.24;
        speedMul=0.94;
      }
    }
  ...
  function drawPlayer(p,view,rank){
    ...
    const r=Math.max(11.75,view.scale*1.48*PLAYER_VISUAL_SCALE);
